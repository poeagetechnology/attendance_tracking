    ...
    <main className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button
            onClick={handleReset}
            className="flex items-center gap-2 px-4 py-2.5 bg-[#1E293B] text-white border border-[rgba(148,163,184,0.2)] hover:border-[#D4AF37] rounded-lg transition-colors font-medium text-sm"
          >
            <RotateCcw className="w-4 h-4" /> Reset
          </button>
          <button
            onClick={handleSave}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg transition-colors font-medium text-sm ${saved ? "bg-emerald-500 text-white" : "bg-[#D4AF37] hover:bg-[#B8941E] text-[#0B0F19]"}`}
          >
        ...
        <div className="bg-[#1E293B] border border-[rgba(148,163,184,0.2)] rounded-xl p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-[#94a3b8]">Branding and identity</p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. AttendPro"
                className="w-full bg-[#0B0F19] text-white placeholder:text-[#334155] px-3 py-2.5 rounded-lg border border-[rgba(148,163,184,0.2)] focus:outline-none focus:ring-2 focus:ring-[#D4AF37] text-sm"
              />
            </div>
            <div>
              <label className="block text-xs text-[#94a3b8] mb-1.5">
              ...
              {DAYS.map((day) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                    form.weekendDays.includes(day)
                      ? "bg-[#D4AF37]/20 text-[#D4AF37] border-[#D4AF37]/40"
                      : "bg-[#0B0F19] text-[#94a3b8] border-[rgba(148,163,184,0.2)] hover:border-[#D4AF37]"
                  }`}
                >
                  {day}
                </button>
        ...
        <div className="bg-[#1E293B] border border-[rgba(148,163,184,0.2)] rounded-xl p-6">
          ...
          <div className="flex items-center justify-between p-4 bg-[#0B0F19] rounded-lg border border-[rgba(148,163,184,0.2)]">
            <div>
              ...
              <p className="text-xs text-[#94a3b8]">
                Receive alerts for late arrivals and leave requests
              </p>
            </div>
            <button
              onClick={() =>
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${form.notificationsEnabled ? "left-7" : "left-1"}`}
              />
            </button>
          </div>
        </div>
        ...
        <div className="bg-[#1E293B] border border-red-500/30 rounded-xl p-6">
          ...
          <button
            onClick={() => {
              if (
                window.confirm(
                  "This will erase ALL data (employees, attendance, leave, settings). Cannot be undone. Continue?",
                )
              ) {
                localStorage.clear();
